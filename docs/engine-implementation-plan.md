...
| Change | Why |
|---|---|
| New milestone S (spikes) | Hypotheses #2 and #3 were testable cheaply but gated behind 8 milestones |
| 2.4 rewritten: torn / partial / reordered writeback | All-or-nothing crash model made torn states unreachable, so 2.5 / 4.4 / M8 passed vacuously |
| Fourth frame state "undecodable" added (2.5, 4.1, 4.4) | Torn header is neither zeroed-free nor validly headed; rev 1 asserted it came back "free" when it isn't |
| Old 3.5 split into 3.6 and 3.7 | Its acceptance criterion contradicted itself: "all records present" *and* "not all present" |
| New 3.3 and 4.3 (cross-frame transactions) | "Only ever needs checking at the tail" is per-frame; transactions span frames, and a sealed frame can hold an uncommitted prefix |
| Sentinel zero (1.3) | Makes "all-zero slot never validates" structural; 4.2's append-point scan silently depended on it |
| 1.2: salt prepended; hash appended after data, not in the header | crc32c is linear, so an algebraic salt combine is forgeable; the hash's position and field width were stated inconsistently |
| Record header field widths left to implementation | The two documents disagreed on bitmap width and on whether the hash sat inside the header; nothing in the design depends on the specific numbers |
| 4.2: trusting vs. paranoid modes | Was implicit; needed explicitly so 9.2 can measure the delta |
| 0.1/0.2: `zero_range` + `sector_size` on the Device interface | 7.4 needs a portable zeroing path; 2.4 needs sector granularity |
| New 0.6 (extension durability) | `fallocate` isn't durable without fsync; benign but untested |
| 7.3: don't copy above the commit watermark | Follows from 4.3b: eviction could otherwise launder an uncommitted prefix |
| 7.7: `table_version` clause cut | Nothing in draft 1 produces a second schema version; field retained but unused |
| 8.3: coverage histogram required | "10,000 clean seeds" is vacuously satisfiable |
| 4.6: mutation requirement | A test that passes with its mechanism removed isn't testing the invariant |
